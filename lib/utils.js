...
const getOffkabNyaa = async (keyword) => {
  try {
    const nyaaResponse = await gmRequest({
      method: "GET",
      url: `https://sukebei.nyaa.si/user/offkab?f=0&c=0_0&q=${keyword}`,
    });
    const parser = new DOMParser();
    const nyaaDoc = parser.parseFromString(
      nyaaResponse.responseText,
      "text/html",
    );
    const resultTable = nyaaDoc.querySelector("table.torrent-list");

    if (resultTable) {
      return Array.from(resultTable.tBodies[0].rows).map((e) => {
        return {
          title: e.cells[1].children[0].title,
          url: e.cells[1].children[0].pathname,
          mag: e.cells[2].children[1].href,
          date: new Date(e.cells[4].textContent),
        };
      });
    } else {
      return [];
    ...
    console.error("unable to query nyaa", error);
 ...
 */
const getJellyfinRequester = (apiUrl, apiKey, libId) => {
